      ...
      <div className="mx-auto max-w-2xl px-4 md:px-6 py-8 md:py-12">
        ...
        <article className="mt-6 rounded-[21px] border border-edge bg-panel p-fb3">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-edge pt-3">
            <span className="flex items-center gap-1 font-mono">
              <button onClick={() => vote(1)} disabled={tier === "visitor"} aria-label="Upvote" className={`px-1 text-xl leading-none ${tier === "visitor" ? "cursor-not-allowed opacity-30" : myVote === 1 ? "text-gold" : "text-muted hover:text-gold"}`}>▲</button>
          ...
          <div className="mt-4 rounded-2xl bg-obsidian border border-white/10 p-4 text-sm">
            ...
            {b.contact === "wa" && !mine && !myRequest && (
              <div className="flex items-center justify-between gap-3">
                ...
                <button onClick={requestWa} className="shrink-0 rounded-full border border-gold/50 px-5 py-2 text-sm font-semibold text-gold hover:bg-gold hover:text-black transition">Request</button>
              </div>
            )}
            ...
            {b.contact === "wa" && !mine && revealedToMe && (
              <p>WhatsApp: <span className="font-mono font-bold text-gold">{b.wa}</span> <span className="text-muted">— shared with you alone. Don&apos;t spread it.</span></p>
            )}
          </div>
        </article>
        ...
        <section className="mt-6">
            ...
            {[...b.comments].reverse().map((c, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-panel px-4 py-3">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[13px] font-bold text-gold">@{c.by}</span>
                  {c.fork && <span className="rounded-md bg-gold/15 border border-gold/40 px-2 py-0.5 font-mono text-[11px] font-bold text-gold">[FORK] — building on this</span>}
                </div>
          ...
          <form onSubmit={comment} className="mt-4 rounded-3xl border border-white/10 bg-panel p-5">
            <textarea value={text} onChange={(e) => setText(e.target.value)} rows={3} placeholder="Reply as @…" maxLength={1200} className="w-full rounded-2xl border border-white/15 bg-obsidian px-4 py-3 text-sm text-ivory outline-none focus:border-gold" />
            <div className="mt-3 flex items-center justify-between">
              <label className="flex items-center gap-2 font-mono text-xs text-muted">
                <input type="checkbox" checked={fork} onChange={(e) => setFork(e.target.checked)} className="h-4 w-4 accent-[#d4af37]" />
                [FORK] I&apos;m building on this
              </label>
              <button className="rounded-full bg-ivory px-6 py-2.5 text-sm font-semibold text-black hover:bg-gold transition">Reply →</button>
            </div>
