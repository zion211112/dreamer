    ...
    <main className="bg-obsidian text-ivory">
      <div className="mx-auto max-w-xl px-6 py-24 md:py-32 text-center">
        ...
        <form onSubmit={subscribe} className="mt-8 rounded-3xl border border-white/10 bg-panel p-7 text-left">
          ...
          <button className="mt-3 w-full rounded-full bg-gold py-3.5 text-sm font-bold text-black hover:bg-ivory transition">Subscribe →</button>
          {msg && <p className="mt-3 text-sm text-muted">{msg}</p>}
        </form>
        <Link href="/school" className="mt-6 inline-block rounded-full border border-white/20 px-8 py-3.5 text-sm font-semibold hover:border-gold transition">
          Enter the console →
        </Link>
