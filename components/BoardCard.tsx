...
const SEAL: Record<LifeState, { w: number; c: string; tick?: boolean }> = {
  proposed: { w: 16, c: "bg-gold/40" },
  ratified: { w: 42, c: "bg-gold" },
  claimed: { w: 42, c: "bg-gold", tick: true },
  active: { w: 72, c: "bg-teal-300" },
  done: { w: 100, c: "bg-emerald-300/85" },
  parked: { w: 42, c: "bg-dim/50" },
  lapsed: { w: 30, c: "bg-dim/30" }
};

const inField =
  "border-b border-white/10 bg-transparent px-0 py-1.5 font-mono text-[12px] text-ivory outline-none transition focus:border-gold";
const ghost = "font-mono text-[11px] uppercase tracking-[0.2em] text-gold transition hover:text-ivory";
...
function LineNode({
  ...
  return (
    <li className="relative py-2 pl-5">
      <span aria-hidden className="absolute left-0 top-0 h-full w-px bg-white/8" />
      <span aria-hidden className={`absolute left-0 top-2 font-mono text-[11px] ${tone}`}>
        {mark}
      </span>
      <div className="min-w-0">
        <div className="flex flex-wrap items-baseline gap-x-2">
          <span className={`font-mono text-[10px] uppercase tracking-[0.2em] ${subject ? "text-gold" : "text-dim"}`}>
            {tag}
          </span>
    ...
    <article className="border-t border-white/8 py-9">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 font-mono text-[11px] uppercase tracking-[0.14em] text-dim">
        <span className="flex flex-wrap items-center gap-x-2">
          <span className="text-gold/80">{b.risk}</span>
        ...
        <span className="flex items-center gap-3 tabular-nums">
          <button
            ...
            className={`px-1 transition ${myVote === 1 ? "text-gold" : "text-dim hover:text-ivory"}`}
          >
            ▲{up}
          </button>
          <button
            ...
            className={`px-1 transition ${myVote === -1 ? "text-earth" : "text-dim hover:text-ivory"}`}
          >
            ▼{down}
          </button>
        </span>
      ...
      {(b.skills ?? []).length > 0 && (
        <div className="mt-3 flex flex-wrap items-center gap-x-2.5 gap-y-1 font-mono text-[11px] uppercase tracking-[0.18em] text-teal-300/90">
          {(b.skills ?? []).map((k, i) => (
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2.5">
          ...
          <div className="relative h-1.5 flex-1 bg-edge/50">
            <div className={`relative h-full ${seal.c}`} style={{ width: `${seal.w}%` }}>
              {seal.tick && (
                <span aria-hidden className="absolute right-0 top-1/2 h-3 w-1 -translate-y-1/2 bg-teal-300" />
              )}
            </div>
          </div>
        </div>
          ...
          <ul className="mt-2 space-y-1.5">
            {seated.map((c, i) => (
              <li key={i} className="flex flex-wrap items-center gap-x-2 font-mono text-[12px] text-muted">
                <span className={c.role === "reviewer" ? "text-teal-300" : "text-gold"}>{c.role}</span>
      ...
      {att > 0 && (
        <div className="mt-5 border-l-2 border-emerald-300/30 pl-4">
          ...
          <ul className="mt-2 space-y-1.5">
            {(b.attestations ?? []).map((a, i) => (
              <li key={i} className="font-mono text-[12px] text-muted">
                ...
                {a.hall && <span className="ml-1.5 font-mono text-[10px] uppercase text-gold/70">hall</span>}
      ...
      {hasFork && (
        <details className="mt-6 border-t border-white/8 pt-4">
          ...
          <ul className="mt-4 list-none">
            ...
            <LineNode b={b} now={now} mark="●" tone="text-gold" tag="now" subject />
            {children.map((c, i) => {
              ...
              return (
                <LineNode
                  ...
                  now={now}
                  mark={live ? "●" : "○"}
                  tone={live ? "text-teal-300" : "text-teal-300/60"}
                  tag={`fork ${i + 1}`}
                />
              );
            })}
      ...
      <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-3 border-t border-white/8 pt-5">
        {me ? (
          <>
            {canClaim &&
              (form === "claim" ? (
      ...
      {msg && <p className="mt-4 font-mono text-xs text-red-400">{msg}</p>}
    </article>
  );
}