  ...
  return <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,340px)_minmax(0,1fr)]">
    <section className={`${panel} print:hidden`}>
      ...
      <div className="space-y-4">
        ...
        <label className="block text-sm text-muted">Notes · 3–8 lines, Concept: explanation<textarea rows={8} className="mt-2 w-full rounded-2xl border border-edge bg-void p-4 text-sm leading-6 text-ivory outline-none focus:border-gold" value={brief.notes} onChange={e => setBrief({ ...brief, notes: e.target.value })} /></label>
    ...
    <section className={`${panel} min-h-[440px] print:border-0 print:bg-white print:text-black`}>
      ...
      {!test ? <><h2 className="mt-8 font-display text-3xl">Not just “right or wrong”.<br />Find the idea that needs care.</h2><p className="mt-5 max-w-md text-sm leading-7 text-muted">Your child answers a few questions. You get explanations and a small next step to try together.</p><p className="mt-10 text-sm leading-7 text-gold">Is half a small chapati the same amount as half a large one? Understanding the whole changes everything.</p></> : <>
        ...
        <pre className="print-sheet hidden whitespace-pre-wrap bg-white p-8 font-body text-sm leading-8 text-black print:block">{testDocument(test.brief, test.questions, false)}</pre>
        <div className="space-y-6 print:hidden">
          {test.questions.map((q, i) => <fieldset key={i} disabled={submitted} className="rounded-2xl border border-edge p-5">
            ...
            <div className="space-y-2">{q.options.map((option, j) => <label key={j} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 text-sm leading-6 ${answers[i] === j ? "border-gold bg-gold/5" : "border-edge"}`}><input className="mt-1.5 accent-[#d4af37]" type="radio" name={`answer-${i}`} checked={answers[i] === j} onChange={() => setAnswers(answers.map((a, n) => n === i ? j : a))} />{option}</label>)}</div>
            {submitted && <p className="mt-4 text-sm leading-6 text-muted"><strong className="text-gold">{answers[i] === q.correct ? "Got it. " : "Let's revisit. "}</strong>{q.explanation}</p>}
          </fieldset>)}
          {!submitted ? <button className={btn} disabled={answers.some(a => a < 0)} onClick={() => setSubmitted(true)}>Check answers · {answers.filter(a => a >= 0).length}/{test.questions.length}</button> : result && <div role="status" className="rounded-2xl border border-gold/30 bg-gold/5 p-6"><h3 className="font-display text-2xl">{result.correct} of {result.total} understood this time.</h3><p className="mt-3 text-sm leading-7 text-muted">{result.revisit.length ? `Practise next: ${result.revisit.join(", ")}. Pick one idea, draw it together, then ask your child to explain it back.` : "Next: ask your child to make a new example and teach it to you."}</p><button className={`${btnGhost} mt-4`} onClick={() => { setAnswers(Array(test.questions.length).fill(-1)); setSubmitted(false); }}>Try again</button></div>}
        </div>
      </>}
    </section>
  </div>;
