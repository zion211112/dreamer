    ...
    <div>
      ...
      <div className={panel}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            className="font-mono text-[11px] uppercase tracking-[0.15em] text-dim transition hover:text-gold"
            onClick={() => {
                  ...
                  {parsed.rows.slice(0, 500).map((r, i) => (
                    <tr key={i} className={`border-b border-edge/50 ${r.dupe ? "opacity-40" : ""}`}>
                      ...
                      <td className="px-3 py-2">
                        {r.dupe && (
                          <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-gold">On roll</span>
                        )}
                      </td>
                    </tr>
                  ))}
