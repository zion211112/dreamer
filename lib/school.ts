...
/* ------------------------------------------------------------------ */
/* Class taxonomy: ECDE → Grade → Junior → Form 6. Every class in the */
/* same family; a stream ("· X") refines it. The composed label is    */
/* what a lesson carries; the base class (no stream) is what a        */
/* lesson resolves its roll against, so "Grade 5 · X" still finds the */
/* whole Grade 5 roll.                                                */
/* ------------------------------------------------------------------ */
...
export const CLASS_CATEGORIES: ClassCategory[] = [
  { id: "ecde", label: "ECDE", classes: ["Creche", "Pre-Nursery", "Nursery", "Kindergarten"] },
  {
    id: "grade",
    label: "Grade",
    classes: ["Grade 1", "Grade 2", "Grade 3", "Grade 4", "Grade 5", "Grade 6", "Grade 7", "Grade 8"]
  },
  { id: "junior", label: "Junior", classes: ["JSS 1", "JSS 2", "JSS 3"] },
  { id: "class", label: "Class", classes: ["Form 1", "Form 2", "Form 3", "Form 4", "Form 5", "Form 6"] }
];

export const CLASS_STREAMS = ["A", "B", "X", "Y"];

export function composeClassLabel(cls: string, stream?: string): string {
  return stream ? `${cls} · ${stream}` : cls;
}

// "Grade 5 · X" → "Grade 5". The roll for a lesson is keyed on the base class.
export function baseClassName(label: string): string {
